...
export const ProjectForm = ({ mode, initialData, onSubmit, isLoading }: ProjectFormProps) => {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            ${errors.description ? 'border-red-300 text-red-900 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600'}
          `}
                    placeholder="Describe your project goals..."
                    {...register('description')}
                />
                {errors.description && (
                    <p className="mt-2 text-sm text-red-600 dark:text-red-400 font-medium">{errors.description.message}</p>
                )}
            </div>
